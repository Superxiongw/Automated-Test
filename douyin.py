...
class DouYin:
    ...
    def hello(self):
        ...
        print("*" * 50)
        print(' ' * 15 + '抖音下载小助手')
        print(' ' * 5 + '作者: xx  Date: 2022-09-05 12:00')
        print(' ' * 15 + '无水印 | 有水印')
        ...
        while result == []:
            ...
            if html['aweme_list'] != []:
                ...
                for item in result:
                    ...
                    video_list.append({'desc': re.sub(r'[\/:*?"<>|]', '', item['desc']) if item['desc'] else '无标题' + str(int(time.time())),'url': item['video']['play_addr']['url_list'][0]})
    ...
    def run(self):
        ...
        save_dir = save_dir if save_dir else "E:\2022test"
        ...
        if type_flag == 'p':
            if 'favorite' not in os.listdir(nickname_dir):
                ...
                for num in range(len(video_list)):
                    ...
                    video_path = os.path.join(nickname_dir, video_list[num]['desc']) if type_flag != 'p' else os.path.join(nickname_dir,'favorite',video_list[num]['desc'])
